...
"  Last Modified: 2010-06-13 12:21
"        Version: 2.27
"=============================================================================
scriptencoding utf-8

"キーマップリーダーが g の場合、「新規ファイルを作成」は g,c です。
"簡単な使い方はg,,のメニューで、詳しい使い方は以下のサイトを参照してください。
"http://sites.google.com/site/fudist/Home/qfixhowm
"
"----------以下は変更しないで下さい----------
...
"コマンド定義
if !exists('g:QFixHowm_Cmd_NewTemplate')
  let g:QFixHowm_Cmd_NewTemplate = g:QFixHowm_Title." \<CR>\<C-r>=strftime(\"[".s:hts_dateTime."]\")\<CR>\<CR>"
  let g:QFixHowm_Cmd_NewTemplateP = "I".g:QFixHowm_Cmd_NewTemplate."<CR><ESC><Up><Up><Up>$a"
endif
if !exists('g:QFixHowm_Cmd_New')
  let g:QFixHowm_Cmd_New = "I".g:QFixHowm_Cmd_NewTemplate."\<ESC>\<Up>\<Up>$a"
endif
if !exists('g:QFixHowm_Key_Cmd_C')
  let g:QFixHowm_Key_Cmd_C = "o".g:QFixHowm_Cmd_NewTemplate."<ESC><Up><Up>$a"
endif
if !exists('g:QFixHowm_Key_Cmd_n')
...
if !exists('g:QFixHowm_Key_Cmd_p')
...
if !exists('g:QFixHowm_Key_Cmd_d')
  let g:QFixHowm_Key_Cmd_d = "i<C-r>=strftime('[".s:hts_date."]')<CR>"
endif
if !exists('g:QFixHowm_Key_Cmd_T')
  let g:QFixHowm_Key_Cmd_T = "i<C-r>=strftime('[".s:hts_dateTime."]')<CR>"
endif
...
if g:QFixHowm_Default_Key > 0
  ...
  "exec 'silent! nnoremap <unique> <silent> '.s:QFixHowm_Key.'u :<C-u>call QFixHowmOpenQuickMemo(g:QFixHowm_QuickMemoFile)<CR>'
...
function! s:SetbuflocalKey()
  ...
  exec "silent! nnoremap <unique> <silent> <buffer> ".s:QFixHowm_Key."d " . g:QFixHowm_Key_Cmd_d
  exec "silent! nnoremap <unique> <silent> <buffer> ".s:QFixHowm_Key."T " . g:QFixHowm_Key_Cmd_T
  exec "silent! nnoremap <unique> <silent> <buffer> ".s:QFixHowm_Key."C " . g:QFixHowm_Key_Cmd_C
  exec "silent! nnoremap <unique> <silent> <buffer> ".s:QFixHowm_Key."n " . g:QFixHowm_Key_Cmd_n
  exec "silent! nnoremap <unique> <silent> <buffer> ".s:QFixHowm_Key."N " . g:QFixHowm_Key_Cmd_N
  exec "silent! nnoremap <unique> <silent> <buffer> ".s:QFixHowm_Key."p " . g:QFixHowm_Key_Cmd_p
  exec "silent! nnoremap <unique> <silent> <buffer> ".s:QFixHowm_Key."P " . g:QFixHowm_Key_Cmd_P
...
if QFixHowm_MenuBar
  ...
  if g:QFixHowm_UseMRU
    ...
    exec 'amenu <silent> 41.332 '.s:menu.'.RebuildKeywordFile(&B)<Tab>'.s:QFixHowm_Key.'rk  :<C-u>call QFixHowmRebuildKeyword()<CR>'
    exec 'amenu <silent> 41.332 '.s:menu.'.RebuildRandomWalkFile(&B)<Tab>'.s:QFixHowm_Key.'rR  :<C-u>call QFixHowmRebuildRandomWalkFile(g:QFixHowm_RandomWalkFile)<CR>:QFixHowmRandomWalk<CR>'
...
augroup QFixHowm
  autocmd!
  exec "autocmd BufReadPre   *.".g:QFixHowm_FileExt." silent! exec 'call QFixHowmInit()'"
  exec "autocmd BufReadPost  *.".g:QFixHowm_FileExt." if g:QFixHowm_ForceEncoding |exec 'edit! ++enc='.g:howm_fileencoding.' ++ff='.g:howm_fileformat |endif |call QFixHowmBufReadPost()"
...
"  vnoremap <buffer> <silent> X  :call QFixHowmCmd_X('move')<CR>
  ...
  exec 'silent! nnoremap <buffer> <silent> '.s:QFixHowm_Key.'w :call MyGrepWriteResult(0, "")<CR>'
  exec 'silent! vnoremap <buffer> <silent> '.s:QFixHowm_Key.'w :call MyGrepWriteResult(1, "")<CR>'
endfunction
...
function! QFixHowmCmd_CR(mode) range
  ...
  if a:firstline == a:lastline
    ...
    let mes = printf("Open files: Are you sure?")
...
function! QFixHowmCmd_X(...) range
  ...
  let mes = "!!!Delete Entry! Are you sure?"
  if a:0 > 0
    let mes = "!!!Move Entry! Are you sure?"
  endif
  let choice = confirm(mes, "&Yes\n&Cancel", 2, "W")
  if choice == 1
...
function! QFixHowmCmd_RD(cmd) range
  ...
  if a:firstline != a:lastline
    if a:cmd == 'Delete'
      let mes = "!!!Delete file(s): Are you sure?"
    else
      let mes = "!!!Remove to (~howm_dir) : Are you sure?"
    endif
    let choice = confirm(mes, "&Yes\n&Cancel", 2, "W")
    if choice != 1
      return
  ...
  for index in range(1, loop)
    ...
    if a:firstline == a:lastline
      if a:cmd == 'Delete'
        let mes = "!!!Delete : ".file.". Are you sure?"
      else
        let mes = "!!!Remove to (~howm_dir) : ".file.". Are you sure?"
      endif
      let choice = confirm(mes, "&Yes\n&Cancel", 2, "W")
    endif
    if choice != 1
...
function! QFixHowmCreateNewFile(...)
  ...
  if expand('%') =~ l:MyOpenVim_ExtReg
    let g:QFixHowm_LastFilename = ' ' . g:howm_glink_pattern . ' ' . expand('%:p')
  endif
  if a:0 > 0
  ...
  silent! exec 'normal! '. g:QFixHowm_Cmd_New
  if g:QFixHowm_Cmd_New =~ 'a$'
    startinsert!
  endif
  if g:QFixHowm_Cmd_New =~ 'i$'
    startinsert
  endif
  call QFixHowmHighlight()
  return
...
function! QFixHowmInit()
  ...
  "FIXME:これは削除する？
"  call QFixPermanentWindow(title, cmd)
...
function! s:OpenUri(uri)
  ...
  if g:QFixHowm_removeHatenaTag
    let uri = substitute(a:uri, ':title=.*$', '', '')
  endif
...
function! QFixHowmOpenCursorline()
  ...
  let lstr = strpart(str, col('.'))
  let lstr = matchstr(lstr, '^'.pathchr.'\+')
  let str = strpart(str, 0, col('.'))
  let str = matchstr(str, pathchr.'\+$')
  let str = str . lstr

  let pathhead = '\([A-Za-z]:[/\\]\|\~/\)'
  let urireg = '\(\(howm\|rel\|http\|https\|file\|ftp\)://\|'.pathhead.'\)'.pathchr.'\+'
  let str = matchstr(str, urireg)
  if str !~ '^'.urireg.'$'
    return "\<CR>"
  endif
  let relpath = g:QFixHowm_RelPath . (str =~ 'rel://[^/\\]' ? '/' : '')
...
"    let dir = expand(dir)
    ...
    exec 'edit '.efile
    silent exec 'set fenc=' . g:howm_fileencoding
    silent exec 'set ff='. g:howm_fileformat
...
function! QFixHowmCursor(pos, ...)
  ...
  for i in range(1,cnt)
    ...
    if a:pos == 'prev'
      ...
      let fline = search('^'.l:QFixHowm_Title, 'cnbW')
      if fline == 0
        let fline = line('1')
      else
        let fline = fline
...
silent! function QFixHowmCreateNewFileWithTag(tag)
  ...
  let g:QFixHowm_Cmd_New = "I".title." \<CR>\<C-r>=strftime(\"[".s:hts_dateTime."]\")\<CR>\<CR>\<ESC>\<Up>\<Up>0w"
  call QFixHowmCreateNewFile()
...
silent! function QFixHowmFileList(path, file)
