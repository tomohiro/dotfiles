...
"                 Preview, sorting and advanced search for Quickfix.
"         Author: Futoshi Ueno <fuenor@gmail.com>
"                 http://sites.google.com/site/fudist/Home  (Japanese)
"  Last Modified: 2010-06-03 20:12
"        Version: 2.15
"=============================================================================
"
"What Is This:
"  This plugin adds preview, sorting and advanced search to your quickfix window.
"
"Install:
...
augroup QFix
  ...
  autocmd BufWinEnter             * call <SID>QFixBufWinEnter()
...
function! s:QFixBufEnter(...)
  ...
  if &buftype == 'quickfix'
    ...
    if g:QFix_HighSpeedPreview
      copen
      "FIXME:QFixHowmの色分けのための特殊処理
      if exists('*QFixHowmSetup')
        call QFixHowmSetup('color')
      endif
    endif
...
function! s:QFixSetup(...)
  ...
  nnoremap <buffer> <silent> <CR>   :call <SID>QFixMoveCursor()<CR><CR>z.:call <SID>AfterJump()<CR>
  nnoremap <buffer> <silent> <S-CR> :call <SID>QFixMoveCursor()<CR>:call <SID>QFixSplit()<CR>z.:call <SID>AfterJump()<CR>
  if g:QFix_Edit == 'tab'
    nnoremap <buffer> <silent> <S-CR> :call <SID>QFixMoveCursor()<CR>:call <SID>QFixEdit()<CR>z.:call <SID>AfterJump()<CR>
  endif
  nnoremap <buffer> <silent> <C-w>.     :ResizeOnQFix<CR>
  nnoremap <buffer> <silent> <C-w><C-r> :ResizeOnQFix<CR>
...
function! QFixCopen(cmd, mode)
  ...
  if idx < 0
    let g:QFix_SearchPath = ''
    let g:QFix_SelectedLine = 1
  elseif g:QFix_SearchPath != ''
    "登録されている半分のファイルが QFix_SearchPath以下になかったらクリア
    let none = 0
    for n in qf
      let file = fnamemodify(bufname(n['bufnr']), ':p')
      let file = expand(substitute(file, '\\', '/', 'g'))
      if file == '' || file !~ '\c'.expand(substitute(g:QFix_SearchPath, '\\', '/', 'g'))
        let none += 1
      endif
    endfor
    if none > (idx / 2)
...
function! QFixPreview()
  ...
  if g:QFix_HighSpeedPreview
    ...
    let file = g:QFix_HSPSearchPath . '/' . gfile
...
"  endif
  let s:result = []
  call add(s:result, g:QFix_SearchPath . '|'.line('.').'|')
